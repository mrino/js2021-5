...
express(): 서버 애플리케이션을 생성한다.  
use(): 요청이 왔을 때 실행할 함수를 지정한다.  
listen(): 서버를 실행한다.
...
npm install <모듈이름>을 cmd에 치면 외부 모듈을 설치할 수 있다.
가져온 외부 모듈들은  
underscore, express, request, cheerio 등 가져왔다.
